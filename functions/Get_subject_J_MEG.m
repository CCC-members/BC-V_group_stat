...
clc;
%
BCV_output_path = "/data3_260T/data/CCLAB_DATASETS/HCP-GUSHI-2019/Run_1/BC_VARETA_OutPut_8K/All_data_bin";
...
for i=1:length(methods)
    %     J3Dnorm         = zeros(8002,length(binds),length(subjects));
    %     J3Dnorm_sp      = zeros(8002,length(binds),length(subjects));
    %     J3D             = zeros(8002,length(binds),length(subjects));
    %     J3Dsp           = zeros(8002,length(binds),length(subjects));
    %     F3Dstat         = zeros(8002,length(binds),length(subjects));
    %
    %     J3Dnorm_fsa     = zeros(8002,length(binds),length(subjects));
    %     J3Dnorm_sp_fsa  = zeros(8002,length(binds),length(subjects));
    %     J3D_fsa         = zeros(8002,length(binds),length(subjects));
    %     J3Dsp_fsa       = zeros(8002,length(binds),length(subjects));
    %     F3Dstat_fsa     = zeros(8002,length(binds),length(subjects));
    ...
    for j=1:length(subjects)
        ...
        % Native space
        J3Dnorm(j).SubID  = subject.name;
        J3Dnorm(j).SamplingFrequency = meg.data.fsample;
        J3Dnorm(j).Age = info.Age;
        J3Dnorm(j).Sex = info.Sex;
        J3Dnorm(j).Alpha_Peak = info.Alpha_Peak;
        J3Dnorm(j).Beta = info.Beta_Peak;
        J3Dnorm(j).J2Dnorm  = J2Dnorm;
        
        J3Dnorm_sp(j).SubID  = subject.name;
        J3Dnorm_sp(j).SamplingFrequency = meg.data.fsample;
        J3Dnorm_sp(j).Age = info.Age;
        J3Dnorm_sp(j).Sex = info.Sex;
        J3Dnorm_sp(j).Alpha_Peak = info.Alpha_Peak;
        J3Dnorm_sp(j).Beta = info.Beta_Peak;
        J3Dnorm_sp(j).J2Dnorm_sp    = J2Dnorm_sp;
        
        J3D(j).SubID  = subject.name;
        J3D(j).SamplingFrequency = meg.data.fsample;
        J3D(j).Age = info.Age;
        J3D(j).Sex = info.Sex;
        J3D(j).Alpha_Peak = info.Alpha_Peak;
        J3D(j).Beta = info.Beta_Peak;
        J3D(j).J2D      = J2D;
        
        J3Dsp(j).SubID  = subject.name;
        J3Dsp(j).SamplingFrequency = meg.data.fsample;
        J3Dsp(j).Age = info.Age;
        J3Dsp(j).Sex = info.Sex;
        J3Dsp(j).Alpha_Peak = info.Alpha_Peak;
        J3Dsp(j).Beta = info.Beta_Peak;
        J3Dsp(j).J2Dsp  = J2Dsp;
        
        F3Dstat(j).SubID  = subject.name;
        F3Dstat(j).SamplingFrequency = meg.data.fsample;
        F3Dstat(j).Age = info.Age;
        F3Dstat(j).Sex = info.Sex;
        F3Dstat(j).Alpha_Peak = info.Alpha_Peak;
        F3Dstat(j).Beta = info.Beta_Peak;
        F3Dstat(j).F2Dstat  = F2Dstat;
        
        % FSAve space
        J3Dnorm_fsa(j).SubID  = subject.name;
        J3Dnorm_fsa(j).SamplingFrequency = meg.data.fsample;
        J3Dnorm_fsa(j).Age = info.Age;
        J3Dnorm_fsa(j).Sex = info.Sex;
        J3Dnorm_fsa(j).Alpha_Peak = info.Alpha_Peak;
        J3Dnorm_fsa(j).Beta = info.Beta_Peak;
        J3Dnorm_fsa(j).J2Dnorm_fsa  = J2Dnorm_fsa;
        
        J3Dnorm_sp_fsa(j).SubID  = subject.name;
        J3Dnorm_sp_fsa(j).SamplingFrequency = meg.data.fsample;
        J3Dnorm_sp_fsa(j).Age = info.Age;
        J3Dnorm_sp_fsa(j).Sex = info.Sex;
        J3Dnorm_sp_fsa(j).Alpha_Peak = info.Alpha_Peak;
        J3Dnorm_sp_fsa(j).Beta = info.Beta_Peak;
        J3Dnorm_sp_fsa(j).J2Dnorm_sp_fsa    = J2Dnorm_sp_fsa;
        
        J3D_fsa(j).SubID  = subject.name;
        J3D_fsa(j).SamplingFrequency = meg.data.fsample;
        J3D_fsa(j).Age = info.Age;
        J3D_fsa(j).Sex = info.Sex;
        J3D_fsa(j).Alpha_Peak = info.Alpha_Peak;
        J3D_fsa(j).Beta = info.Beta_Peak;
        J3D_fsa(j).J2D_fsa  = J2D_fsa;
        
        J3Dsp_fsa(j).SubID  = subject.name;
        J3Dsp_fsa(j).SamplingFrequency = meg.data.fsample;
        J3Dsp_fsa(j).Age = info.Age;
        J3Dsp_fsa(j).Sex = info.Sex;
        J3Dsp_fsa(j).Alpha_Peak = info.Alpha_Peak;
        J3Dsp_fsa(j).Beta = info.Beta_Peak;
        J3Dsp_fsa(j).J2Dsp_fsa  = J2Dsp_fsa;
        
        F3Dstat_fsa(j).SubID  = subject.name;
        F3Dstat_fsa(j).SamplingFrequency = meg.data.fsample;
        F3Dstat_fsa(j).Age = info.Age;
        F3Dstat_fsa(j).Sex = info.Sex;
        F3Dstat_fsa(j).Alpha_Peak = info.Alpha_Peak;
        F3Dstat_fsa(j).Beta = info.Beta_Peak;
